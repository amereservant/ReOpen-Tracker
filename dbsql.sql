...
CREATE TABLE peers (
  ...
  left INTEGER(20) NOT NULL default 0,
...
/** MySQL SQL **/
DROP TABLE IF EXISTS peers;
CREATE TABLE `tmp`.`peers` (`info_hash` BINARY(20) PRIMARY KEY NOT NULL, 
    `ip` INT(11) NOT NULL, `port` INT(5) NOT NULL, 
    `peer_id` BINARY(20) NOT NULL, 
    `uploaded` INT(20) NOT NULL DEFAULT 0, 
    `downloaded` INT(20) NOT NULL DEFAULT 0, 
    `left` INT(20) NOT NULL DEFAULT 0, 
    `update_time` INT NOT NULL, 
    `expire_time` INT NOT NULL, 
    PRIMARY KEY (`info_hash`)) ENGINE = MyISAM;
